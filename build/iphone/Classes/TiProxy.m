...
//Rare exceptions to indicate a bug in the _vietnews code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_vietnews = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_vietnews]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.vietnews.module"
